...
public class TransaksiDaoImpl implements TransaksiDao{
	...
	public List<Transaksi> listTransaksi(int id_nasabah) {
		...
		List<Transaksi> transaksiList = session.createQuery("from Transaksi where EXTRACT(MONTH FROM tanggal_transaksi)="+ month
				+ " AND EXTRACT(YEAR FROM tanggal_transaksi)=" + year
